...
const Collection1 = () => {
    ...
    {sortedProducts.filter((product) => product.collection === "Valantine").length === 0 ? (
        ...
        <div className="col-span-2 md:col-span-5 text-center text-gray-600 text-lg font-semibold">
            No products available in the "Valantine" collection. 💔
        </div>
    ) : (
        // ✅ Render products normally
        sortedProducts
            .filter((product) => product.collection === "Valantine")
            .map((product, index) => (
                <div
                    key={product.id}
                    className={`bg-white shadow-lg overflow-hidden transition-transform duration-300 w-full ${
                        product.discount > 1 ? "hover:shadow-2xl transform hover:scale-105 text-gray-900" : ""
                    }`}
                >
                    {/* Product Image */}
                    <div className="relative group">
                        <img
                            ...
                            className={`w-full cursor-pointer h-44 object-cover transition-transform duration-300 group-hover:scale-105 font-goudy ${
                                product.discount > 1 ? "h-40 md:h-60" : ""
                            }`}
                            onError={(e) => {
